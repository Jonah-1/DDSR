...
def render_novel_views(model, recontrast_data, render_data, device, scene_name, sample_idx, save_dir, actual_sample_idx, translation_distances=[1.0, 2.0], eval_resolution='280x518', novel_render_frames=[], hoodline_masks=None):
    ...
    for transform_name, transform_matrix in translation_transforms.items():
        ...
        for frame_id in novel_render_frames:
            for cam_id in range(num_cams):
                ...
                if hoodline_masks is not None and cam_id < hoodline_masks.shape[1]:
                    hood_mask = hoodline_masks[0, cam_id, 0].to(render_rgb.device)  # (H, W)
                    gt0_key = ('groudtruth', 0, cam_id)
                    if gt0_key in render_data:
                        gt0 = render_data[gt0_key][sample_idx].clamp(0, 1)  # (3, H, W)
                        render_rgb = render_rgb * (1 - hood_mask.unsqueeze(0)) + gt0 * hood_mask.unsqueeze(0)
...
def _process_scene_batch(model, scene_batch, device, gpu_id=0, save_renders=True, output_dir=None, novel_distances=[1.0, 2.0], eval_resolution='280x518', batch_idx=0, datamodule_type='vggt4dgs'):
    ...
    all_recon_psnr, all_recon_ssim, all_recon_lpips = [], [], []
    ...
    return {
        ...
        'recon_metrics': {
            'psnr_list': all_recon_psnr,
            'ssim_list': all_recon_ssim,
            'lpips_list': all_recon_lpips
        },
        'novel_metrics': {
            'psnr_list': all_novel_psnr,
            'ssim_list': all_novel_ssim,
            'lpips_list': all_novel_lpips
        }
    }
